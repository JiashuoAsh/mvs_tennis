...
4) 避免不必要的新增第三方依赖（确需新增时允许：必须说明必要性/替代方案/影响面，并同步更新依赖声明如 pyproject.toml/锁文件与最小验证）；避免过度设计。
...
- 入口 --help（若存在入口）
- 最小测试（若存在测试框架/测试命令）
- 如果无法验证：写明缺口（例如缺少测试/缺少入口），并给出下一步建议（尽量不引入新依赖；如确需引入，需说明必要性/替代方案/影响面并更新依赖声明与最小验证）
