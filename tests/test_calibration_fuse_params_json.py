"""单测：把内参目录与外参文件融合成 params_4cam_calib 风格 JSON。"""
...
class TestCalibrationFuseParamsJson(unittest.TestCase):
    def test_fuse_matches_repo_reference(self) -> None:
        ...
        # 说明：仓库内的参考文件 params_4cam_calib.json 是通过 tools/generate_params_4cam_calib.py
        ...
        ref_file = repo_root / "data" / "calibration" / "params_4cam_calib.json"

        intr = load_intrinsics_dir(intr_dir)
        extr = load_extrinsics_C_T_B(extr_file)

        # 该映射来源于仓库内的参考文件（params_4cam_calib.json）相机顺序与当时标定流程。
