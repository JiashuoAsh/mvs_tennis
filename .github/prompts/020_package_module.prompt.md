---
name: 020_package_module
description: "把一个完整功能封装成可复用 Python 包：public API + 目录归位 + 可验证用例"
argument-hint: "feature=...; paths=...; breaking=1/0（默认1）"
---

目标功能边界（用一句话描述）：${input:feature:例如“网球检测与定位”}
涉及的代码路径（逗号分隔）：${input:paths:例如 src/foo, app/main.py, scripts/debug_x.py}
是否允许破坏式变更：${input:breaking:1=允许修改对外 API 并清零旧路径（默认1）；0=保持对外入口尽量不变}

目标：把上述功能封装成“可复用包/模块”，让其他程序能稳定地 import 调用，同时保持行为不变。

变更策略：
- breaking=1（默认）：允许修改模块结构与对外 API；必须同步更新仓库内所有调用点，确保旧路径/旧符号清零。
- breaking=0：尽量保持对外入口（import 路径/主要符号/调用方式）不变；允许内部拆分与目录归位；不强制新增专门的转发层。
...
B) 包化设计（以可用/清晰/可验证为目标，避免过度设计；不强求“越小越好”）
...
- 默认不新增第三方依赖；当收益明显时允许：必须说明必要性/替代方案/影响面，并同步更新依赖声明如 pyproject.toml/锁文件与最小验证；不引入复杂框架/插件系统

D) 可用性验证（必须给出）
- 提供“最小使用示例”（别的程序如何 import 并调用 public API）
- 如果仓库支持可编辑安装：给出“可编辑安装后”的调用示例（安装方式按仓库约定，例如 pip/uv 等）
