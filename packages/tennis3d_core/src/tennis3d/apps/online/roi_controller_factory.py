...
职责：
- 在相机已经打开（可读写节点）后，按 `OnlineRunSpec` 构建 ROI 控制器实例。
- 支持：
  - 仅 software crop：`KinematicRoiController`
...
def build_roi_controller(*, spec: OnlineRunSpec, cap: Any, binding: Any):
    ...
    - 两级 ROI：TwoStageKinematicRoiController

    注意：两级 ROI 需要读取每台相机的 Width/Height/Offset 范围，必须在相机打开后构建。
    """
