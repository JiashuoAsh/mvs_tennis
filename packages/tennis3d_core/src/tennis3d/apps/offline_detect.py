"""命令行入口：三相机离线检测（按时间戳对齐 + 推理）。

说明：
- 这是“应用层”入口：只负责参数解析与调用库函数。
- 核心逻辑在 tennis3d.offline_detect.pipeline.run_pipeline。
"""
...
from tennis3d.offline_detect.pipeline import run_pipeline
