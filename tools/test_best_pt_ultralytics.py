...
# 说明：tools/*.py 通常直接用 `python tools/xxx.py` 运行。
# 这时 Python 不会自动把 src/ 加入 sys.path，导致 `import tennis3d` 失败。
# 这里显式把 src/ 加到 sys.path，保证工具脚本可直接运行。
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 单图模式：保持原脚本行为（输出一个 JSON），可选保存可视化。
