...
"""分析 mvs_quad_capture 的采集结果（metadata.jsonl + 输出目录）。

本脚本定位：
- 输入：一次采集输出目录（包含 metadata.jsonl 和 trigger_*/ 图片或 raw 文件）。
- 输出：一份人类可读的诊断报告（可选写出 JSON 汇总）。

为什么需要它：
- 采集成功不等于“同步成功”。需要用一组可量化指标来判断：
  1) 每次触发是否都凑齐 N 台相机（组完整性）。
  2) 网络是否丢包（lost_packet）。
  3) trigger_index 是否正常递增（用于严格分组/证明同一次触发）。
  4) 实际 FPS 是否接近期望。
  5) 组内时间差是否稳定（诊断上位机抖动/排查瓶颈）。
...
def _continuity_gaps(nums: Sequence[int]) -> List[Tuple[int, int]]:
    """返回断档区间列表，例如 [(10, 12)] 表示 10->12 中间缺了 11。"""
...
def _continuity_gaps_ignore_resets(nums_in_time_order: Sequence[int]) -> List[Tuple[int, int]]:
    ...
    说明：metadata.jsonl 允许追加多次运行结果。新一轮采集时 frame_num 可能从 1 重新开始，
    这属于回绕/重启，不应当当作缺帧断档。
    """
...
def analyze_output_dir(
    *,
    ...
    group_by_values = sorted(
        {str(r.get("group_by")).strip() for r in group_records if str(r.get("group_by", "")).strip()}
    )
    ...
    # arrival_monotonic：Grabber 线程记录的“到达主机的单调时间”（秒）
    # - 用它估计 FPS 能尽量绕开保存/写盘耗时
    # - 仍然反映“主机侧最终拿到的帧率”（如果队列满导致丢帧，这里会变低）
    arrival_by_cam: Dict[int, List[float]] = {}
    group_arrival_median: List[float] = []

    # 逐相机明细：用于更“落地”的诊断（相机数少时更有价值）
    ...
    # 相机侧时间戳基线（用于把每台相机的 dev_timestamp 归一化到同一“相对时间”）
    base_dev_by_cam: Dict[int, int] = {}

    # 输出目录结构：trigger_* / group_* 子目录数量（用于辅助理解保存策略）
    try:
    ...
    # frame_num 归一化基线与组内一致性（用于评估 frame_num/sequence 分组的可靠性）
    ...
    for r in group_records:
        ...
        for fr in frs:
            ...
            # frame_num 归一化：每台相机减去其首次 frame_num，再看组内是否一致
            try:
                cam_idx = int(fr.get("cam_index"))
            ...
            if file_rel:
                p = Path(str(file_rel))
                # metadata 写的是相对路径（通常带 captures_master_slave\\...），这里相对 output_dir 上一级更稳
                if not p.is_absolute():
                    # 优先按 repo 根相对（当前工作目录），其次按 output_dir 归一
                    if p.exists():
                        pass
                    else:
                        # 有些情况下会从其它 cwd 运行分析脚本，这里尝试“以 output_dir 为基准”补全路径。
                        alt_candidates = []
                        # 如果 file 字段本身不是以 output_dir.name 开头，尝试 output_dir / file
                        if p.parts and output_dir.name and p.parts[0] != output_dir.name:
                            alt_candidates.append((output_dir / p).resolve())
                        # 常见情况：file 形如 captures_master_slave\\...，以 output_dir.parent 补全
                ...
                # save_mode=none 时 file 可能为 None，这里不算缺失
                pass
        ...
        if group_arrivals:
            # 用组内到达时间的中位数表示“该次触发在主机侧被观测到的时间点”
    ...
    # FPS：用 arrival_monotonic 的差值估计（更贴近“相机实际出图/触发频率”）
    ...
    # 事件：曝光（ExposureStart 优先，其次 ExposureEnd）
    ...
    for cam_idx in sorted(series_by_cam.keys()):
        ...
        # frame_num：用 arrival 时间恢复顺序，统计回绕/断档
    ...
    # 逐序列号曝光 FPS 明细（ExposureStart 优先）
    ...
    lines.append(
        "- 含义：unique=1 表示所有帧一致；>1 说明多相机配置不一致或某些帧元信息异常。"
    )
    lines.append("")
    ...
    lines.append(
        "- 含义：GigE 场景下丢包意味着链路不稳或带宽/包大小设置不佳，可能导致图像损坏或组包超时。"
    )
    lines.append("")
    ...
    lines.append(
        "- 含义：frame_num 断档通常意味着取流/队列丢帧；即使组包完整，也可能出现错位或遗漏。"
    )
    lines.append("")
    ...
    lines.append(
        f"- groups_with_norm_mismatch: {summary.groups_with_frame_num_norm_mismatch}/{summary.records}"
    )
    lines.append(
    ...
    lines.append(
        "- 含义：该指标反映“主机收到三张图的时间差”，受线程调度/网络/磁盘影响；只用于诊断，不等价于曝光不同步。"
    )
    lines.append("")
    ...
    lines.append(
        "- 含义：RAW spread 反映各相机时间戳的绝对偏移；若启用 PTP 且同域同步，RAW spread 通常应接近 0。"
    )
    lines.append(
    ...
    lines.append(
        "- 含义：NORMALIZED 把每台相机的时间戳减去各自的首次值，关注相对变化；该 spread 更接近“同一次触发是否对齐”。"
    )
    lines.append("")
    ...
    lines.append(
        f"- exposure event: {summary.exposure_event_name or '-'} events: {summary.exposure_events}"
    )
    ...
    payload: Dict[str, Any] = {
        ...
        "soft_trigger_targets": {
            k: int(v) for k, v in sorted(soft_send_targets_count.items())
        },
        "checks": [
            {
                "name": name,
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="分析 mvs_quad_capture 的输出目录（metadata.jsonl）")
    p.add_argument(
        "--output-dir",
        required=False,
        default="data/captures",
        # default="captures_master_slave",
    ...
    p.add_argument(
        ...
        help="将汇总写入 JSON 文件（例如 analysis_summary.json）",
    )
